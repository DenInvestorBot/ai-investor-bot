...
def format_advice(symbol: str, timeframe: str, rec: dict, candle_time) -> str:
    ...
    if a == "buy":
        return (f"🤖 Советник · {symbol} · {timeframe}\n"
                ...
                f"Уровни: SL={rec['sl']:.2f}, TP={rec['tp']:.2f} (RR≈{rec['rr']:.1f})\n"
                f"Свеча закрыта: {t}")
    if a == "sell":
        return (f"🤖 Советник · {symbol} · {timeframe}\n"
                f"Рекомендация: ПРОДАВАТЬ/ШОРТ (тренд ↓)\n"
                f"Причина: {rec['reason']}\n"
                f"Уровни: SL={rec['sl']:.2f}, TP={rec['tp']:.2f} (RR≈{rec['rr']:.1f})\n"
                f"Свеча закрыта: {t}")
    if a == "reduce_or_exit":
        return (f"🤖 Советник · {symbol} · {timeframe}\n"
                f"Рекомендация: СНИЖАТЬ ПОЗИЦИЮ/ВЫХОДИТЬ (тренд ↓)\n"
                f"Причина: {rec['reason']}\n"
                f"Свеча закрыта: {t}")
    if a == "wait_pullback":
        return (f"🤖 Советник · {symbol} · {timeframe}\n"
                f"Рекомендация: ЖДАТЬ ОТКАТА (тренд ↑)\n"
                f"Причина: {rec['reason']}\n"
                f"Свеча закрыта: {t}")
    return (f"🤖 Советник · {symbol} · {timeframe}\n"
            f"Рекомендация: НЕТ ДЕЙСТВИЙ\n"
